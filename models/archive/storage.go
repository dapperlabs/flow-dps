...
type ReadLibrary interface {
	...
	LookupPayloadPathsForHeight(height uint64, path *[]ledger.Path) func(*badger.Txn) error
...
type WriteLibrary interface {
	...
	IndexPayloadPathsForHeight(height uint64, path []ledger.Path) func(*badger.Txn) error
